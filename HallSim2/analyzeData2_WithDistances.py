...
os.chdir("C:\Users\Jenna\Documents\Visual Studio 2012\Projects\HallSim2\Output\Working1") #Changes to the director with the input files
os.makedirs(newLocation) #Make the directory for the output
newLocation = newLocation + "\\"
folder = "C:\Users\Jenna\Documents\Visual Studio 2012\Projects\HallSim2\Output\Working1"
...
for filename in os.listdir (folder):
   ...
   # pdb.set_trace()
    ...
    picturefilename = newLocation + 'picture_' + filename[:-4] + '.png'
    #Save the picture 
    frame.save(picturefilename)
        
    #Loop through all of the keys in the phenoIJ dictionary
    #Each key has an assiciated list
    #Do a spatial anlysis on that list
    #Plot the histgram for it
    # Save the histogram
    keys = phenoIJ.keys()
    for mut in keys:
        points = phenoIJ[mut]
        distro = scipy.spatial.distance.pdist(points, 'euclidean')
        n, bins, patches = plt.hist(distro, 10000, normed=0, facecolor='green')
        plt.xlabel('distance')
        plt.ylabel('occurance')
        title = "Mutation: " + str(mut)
        plt.title(title)
        picturefilename2 = newLocation + 'hist' + '_' + str(mut) + "_" + filename[:-4] + '.png'
        plt.savefig(picturefilename2)
...
#   #         
    ...
    summary = total + cancer + aliveString + deadString + reg + unique + breakdown + "Individual: " + first + second + third + fourth + fifth + seventh + eigth
